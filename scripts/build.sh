...
build_talloc() {
    ...
    local cc="$(command -v clang)"
    local ar="$(command -v llvm-ar)"
    local ranlib="$(command -v llvm-ranlib)"
...
build_proot() {
    ...
    local cc="$(command -v clang)"
    local strip="$(command -v llvm-strip)"
    local objcopy="$(command -v llvm-objcopy)"
    local objdump="$(command -v llvm-objdump)"
    ...
    local cflags="--target=${triple}${API_LEVEL} --sysroot=${sysroot} -I${sysroot}/usr/include"
